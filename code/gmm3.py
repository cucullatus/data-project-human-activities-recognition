...
##		"""
        ...
        while True:
            ...
            if (self.L - self.Lprev < self.threshold) or it >= 5000:
                #print "self.L, self.Lprev: ", self.L, self.Lprev
                break
            self.Lprev = self.L
    ...
    def init_params(self,X): # X is trainX
        ...
        distmat = np.kron(np.ones((1,self.K)), np.sum(np.multiply(X,X),1)).reshape((-1,self.K)) + np.kron(np.ones((self.N,1)),np.transpose(np.sum(np.multiply(pMiu,pMiu),1))) - 2*X*np.transpose(pMiu)
...
#    X = np.mat([[1,2,3,4],[4,3,2,1],[5,6,7,8],[8,7,6,5],[4,5,6,7]])
    #N,D = np.shape(X)
    for i in range(10):
        gmm = GMM(4,train_data)
        gmm.gmm(train_data,4)
    #    testX = np.mat([[1,2,3,4],[4,3,2,1],[5,6,7,8],[8,7,6,5]])
        hidden_states=predictIndx(gmm,train_data)
        predictValue(gmm, train_data)
        count=0.0
        for index,i in enumerate(hidden_states):
            if i==train_label[index]:
                count+=1
        accuracy=count/len(train_label)
        print accuracy